...
class GpoModelCitationsnews extends JModelList
{
	var $total;
	var $data;
	...
	function readyForPublishing( $oItem )
	{
		...
		{
			$id = $oItem->id;
			$query = "INSERT IGNORE INTO `#__gpo_awaiting_published` (`id`,`ext_id`,`ext_table`)VALUES(NULL ," . $this->_db->quote( $id ) . ",'nc');";
			$this->_db->setQuery( $query );
...
//Delete unpublished item
		...
		//$link = $front_end . JRoute::_('index.php?option=com_gpo&task=citation&type=' . $this->type . '&id=' . $live_id,false );
		$link = Route::link("site", 'index.php?option=com_gpo&task=citation&type=' . $this->type . '&id=' . $live_id);

		$response = new stdClass();
		$response->live_id = $live_id;
		$response->msg = 'Published NCite <b>' . $live_id . '</b> <a href="' . $link . '" target="_blank">click here to view it live</a>';
//maybe change this to a splash screen. Once it works.		
	...
	function deleteUnpublishedById( $id )
	{
		...
		$this->_db->setQuery( $query );
		$ret = $this->_db->execute();
        /* March 28,2011: NOT SURE WHY IT IS RUN, SO DISABLED
	...
	{
		$query = "DELETE FROM `#__gpo_awaiting_published` WHERE `ext_table`='nc' AND `ext_id`=" . $this->_db->quote( $id );
		$this->_db->setQuery( $query );
		$ret = $this->_db->execute();
	}
...
//swap to published table		
	...
	function prepareForLike( $n, $i )
	{
		if( substr($i,0,1) === '"'  && substr($i,-1) === '"' )
		{
			$i = substr($i, 1,strlen($i)-1);
			return "`$n`=$i";
		}else{
			return "`$n` LIKE " . $this->_db->quote( "%" . $i . "%" );
		}
	}
	...
	{
	    $jinput = JFactory::getApplication()->input;
		$search_id = $jinput->getVar('search_id', '', '', 'string');
		$search_ext_id = $jinput->getVar('search_ext_id', '', '', 'string');
		$search_title = $jinput->getVar('search_title', '', '', 'string');
		$where = "";
		if( !empty( $search_id ) )
		{
		...
		{
			$search_ext_id = str_replace( "N",'', $search_ext_id );
			$search_ext_id = str_replace( "n",'', $search_ext_id );
			
			$where = "where `ext_id`=" . $this->_db->quote( $search_ext_id );
		}else if( !empty( $search_title ) )
		{
			$where = "where " . $this->prepareForLike( 'title', $search_title );
		}		
      ...
      $mainframe->redirect( $url );
		...
		$tbl_name = $this->make_table_name( true );
		$query = "SELECT *, CONCAT('N',`ext_id`) as `ext_id` FROM " . $tbl_name;
		$query .= " " . $where;	
		
		$jinput = JFactory::getApplication()->input;
		$orderby = $jinput->getVar('filter_order', '', '', 'string');
		$orderbydir = ( $jinput->getVar('filter_order_Dir', '', '', 'string') === 'asc' ) ? 'ASC' : 'DESC';		
...
//		$query .= " ORDER BY `id` ASC ";		
		$query = "SELECT `c` . * , `ap`.`id` AS `ap_id`,CONCAT('N',`c`.`ext_id`) as `ext_id` FROM " . $tbl_name . " AS `c` LEFT JOIN `#__gpo_awaiting_published` AS `ap` ON `c`.`id` = `ap`.`ext_id` AND `ap`.`ext_table` = 'nc'";
		$query .= " ORDER BY `c`.`modified` ASC";
	...
	function emptyUnpublished()
	{
		...
		$query = "DELETE FROM `#__gpo_awaiting_published` WHERE `ext_table`='nc'";
		$this->_db->setQuery( $query );
		$this->_db->execute();
		...
		$response->link = 'index.php?option=com_gpo&controller=citations&type=' . $this->type;
		return $response;
	}
	...
	function fields( $published=false)
	{
		if( $published === false )
		{
			$fields="id,live_id,published,title,subtitle,source,category,byline,content,websource,entered,modified,notes,share,ext_id";
		}else{
			$fields="id,published,title,subtitle,source,category,byline,content,websource,entered,modified,notes,share,ext_id";
		}
...
//this needs to happen to deal with the format
	...
	/*
	 * Copy from news to citation news
	 */
...
/*	
	function filterByline( $published = true )
	{
		$byline = $jinput->getVar('byline', '', '', 'string');

		$tbl_name = $this->make_table_name( $published );
		$filter		= null;
				
		$query = 'SELECT DISTINCT( `byline` ) AS value, `byline` AS text FROM ' . $tbl_name . ' ORDER BY `byline` ASC';
		// Initialize variables
		$db	= & JFactory::getDBO();
		$items = array();
		$items[] = JHTML::_('select.option', '0', '- '.JText::_('Select Byline').' -');
		$db->setQuery($query);
		$items = array_merge( $items, $db->loadObjectList());

		$html = JHTML::_('select.genericlist',  $items, 'byline', 'class="inputbox" onchange="document.adminForm.submit( );"', 'value', 'text', $byline);
		return $html;
	}
*/
    ...
    function getNextById($id,$state) {
        ...
        $query = "SELECT `id` FROM #__gpo_citations_news".$state." WHERE `id` > " . $this->_db->quote($id) . " ORDER BY `id` ASC LIMIT 1";
        $this->_db->setQuery($query);
        $data = $this->_db->loadObject();
    ...
    function getPrevById($id,$state) {
        ...
        $query = "SELECT `id` FROM #__gpo_citations_news".$state." WHERE `id` < " . $this->_db->quote($id) . " ORDER BY `id` DESC LIMIT 1";
        $this->_db->setQuery($query);
        $data = $this->_db->loadObject();
        if (empty($data->id)) {
            return false;
