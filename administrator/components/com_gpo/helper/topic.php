...
class GpoTopic
{
	private $_table_name = '#__gpo_topic';
	private $_db;
	private $data;
	...
	function load( $input )
	{
		...
		{
			$query = "SELECT * FROM `#__gpo_topic` WHERE `seo`=" . $this->_db->quote( $input ) . " LIMIT 0,1";	
		}else{
			$query = "SELECT * FROM `#__gpo_topic` WHERE `id`=" . $this->_db->quote( $input ) . " LIMIT 0,1";
