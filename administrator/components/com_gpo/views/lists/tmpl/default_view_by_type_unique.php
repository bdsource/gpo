...
foreach( $this->all as $k => $row ):
	...
	foreach( $row['table'] as $table )
	{
		if( $table === 'news' )
		{
			$url = JRoute::_( 'index.php?option=com_gpo&task=search&controller=news&news[' . $this->type . ']=' . $jView->escape( $row['item'] ) );
		}else if( $table === 'quotes' )
		{
			$url = JRoute::_( 'index.php?option=com_gpo&task=search&controller=quotes&quotes[' . $this->type . ']=' . $jView->escape( $row['item'] ) );
		}else if( $table === 'qcite' )
		{
			
			$url = JRoute::_( 'index.php?option=com_gpo&task=search&controller=citations&&type=quotes&citation[' . $this->type . ']=' . $jView->escape( $row['item'] ) );
		}else if( $table === 'ncite' )
		{
			$url = JRoute::_( 'index.php?option=com_gpo&task=search&controller=citations&&type=news&citation[' . $this->type . ']=' . $jView->escape( $row['item'] ) );
		}
...
<script>
var current_list = <?php echo ( !empty( $this->current_list ) ) ? $this->current_list . ";" : "''"; ?>


if( Object.isArray( current_list ) )
{
	$$("td.value").each(function(e){
		var str;
		str = e.innerHTML;
		str = str.unescapeHTML();
		if( current_list.indexOf( str ) !== -1 )
		{
