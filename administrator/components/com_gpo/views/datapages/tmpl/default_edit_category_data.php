...
function getEditCategoryHTML($colKey, $previewDPURL, $colVal, $colValEs, $colValFr) {
    ...
    $catHTML = '
        ...
        <div class="right_col">
            <p class="data">
            <textarea name="data_en_'.$colKey.'" style="width:95%;" class="textarea" rows="2">'.htmlspecialchars($colVal).'</textarea>
            </p>
         
            <p class="data">
            <textarea name="data_es_'.$colKey.'" style="width:95%;" class="textarea" rows="2">'.htmlspecialchars($colValEs).'</textarea>
            </p>
         
            <p class="data">
            <textarea name="data_fr_'.$colKey.'" style="width:95%;" class="textarea" rows="2">'.htmlspecialchars($colValFr).'</textarea>
            </p>
        </div>
...
echo  '<div class="buttonbar">
     ...
     echo '<table class="adminlist" id="adminList" width="1270" style="table-layout:fixed;">
          <thead><tr><th width="10">&nbsp;</th><th width="140">'. JText::_('Category Title').'</th><th width="370">'.JText::_('Data Value').'</th></tr></thead>
          <tbody class="foo">';
     ...
     foreach( $this->dp_metadata as $key=>$val )
     {
       ...
       if( in_array($val, $gatewayColumns) ){
          ...
          //$colRow .= '</br><span id="fr" class="edit_area">'. htmlspecialchars($this->dp_data->{$val}) . '</span>';
          //$colRow .= '</br><span id="{$val}_es" class="edit_area">'. htmlspecialchars($this->dp_data->{$val}) . '</span>';
          $previewMultiDPURL = $this->previewMultiDPURL . "&category=" . urlencode($val); 
          $colRow   .= getEditCategoryHTML($val, $previewMultiDPURL, $this->dp_data->{$val}, 
        ...
        $treeLeaves[] = $val; //push the outputted columns
     	...
     	foreach( $dp_tree['level1'][$val] as $k => $v ) {
                        ...
                        if( in_array($v, $treeLeaves) ) { continue; } //already printed, SKIP.
     		            ...
     		            $treeHtml .= '<tr class="l1 '.$class.' hidden" title="'.$v.'">'
     		                         . '<td width="10">&nbsp;</td>'
     ...
     //echo '<pre>' . print_r($treeLeaves,true) . '</pre>';
...
/* inline edit */ 
 jQuery(document).ready(function() {
     ...
     var postURL = '<?php echo JRoute::_('index.php?option=com_gpo&controller=datapages&task=updateColumnData&lang='.$this->currentLanguage,false);?>';     
