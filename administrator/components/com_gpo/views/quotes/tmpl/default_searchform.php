...
<div style="text-align:center;"><h1 style="font-weight:bold">Search Quotes</h1></div>
...
<div class="row">
	<div class="cell">
		<p>
			<label style="display:block;" for="quotes_published_range_from" title="Published From, Search by Date Published, Enter lower to upper date">Published Range: from</label>
			<input class="input_field published" type="text" readonly id="quotes_published_range_from" name="quotes[published_range][from]" value="<?php echo $this->oQuote->published_range->from; ?>" /><button title="Type in DD-MM-YY, and/or select your date from the Date Picker. Enter lower to upper date" id="quotes_published_range_from_trigger">*</button> 
		</p>
	</div>
	<div class="cell">
		<p>
			<label style="display:block;" for="quotes_published_range_to" title="Published To, Search by Date Published, Enter lower to upper date, If you left empty To Date the system will search up to current date.">Published Range: to</label>
			<input class="input_field published" type="text" readonly id="quotes_published_range_to" name="quotes[published_range][to]" value="<?php echo $this->oQuote->published_range->to; ?>" /><button title="Type in DD-MM-YY, and/or select your date from the Date Picker. Enter lower to upper date" id="quotes_published_range_to_trigger">*</button>
		</p>
	</div>
...
<div class="row">
	<div class="cell">
<?php 
if( $this->oQuote->share == '2' || !isset($this->oQuote->share) ){
        $share_all    = 'checked="true"';
	$share_public = '';
	$share_member = '';
}
else if( $this->oQuote->share == '1' ){
...
?>
	<p>
            <input type="radio" id="quotes_share_all" name="quotes[share]" value="2" <?php echo $share_all; ?>/><label title="Search in all records (both public and members only)" for="quotes_share_all">All</label>
            <input type="radio" id="quotes_share_public" name="quotes[share]" value="1" <?php echo $share_public; ?>/><label title="Publish to GPO open web site." for="quotes_share_public">Public</label>
            <input type="radio" id="quotes_share_member" name="quotes[share]" value="0" <?php echo $share_member; ?>/><label title="Restrict to Members-only web pages (default)." for="quotes_share_member">Members</label>
	</p>
	</div>
    <div class="cell" style="margin-left:100px">
        <p>
            <label title="Search by staff member initials (2010 and later)">Staff <input type="text" name="quotes[staff]" /></label>
...
$('select_quotes_city').observe( 'change', function(event){
	if( this.selectedIndex != '0' )
	{
		select_split( 'select_quotes_city', 'quotes_city' );
	}
...
//county - mouse click & key = return and onChange
...
$('select_quotes_country').observe( 'change', function(event){
	if( this.selectedIndex != '0' )
	{
		location_select_add( this.readAttribute('id') );	
	}
...
//region - mouse click & key = return and onchange
...
$('select_quotes_region').observe( 'change', function(event){
	if( this.selectedIndex != '0' )
	{
		location_select_add( this.readAttribute('id') );	
	}
...
$('select_quotes_keywords').observe( 'change', function(event){
	if( this.selectedIndex != '0' )
	{
		select_split( 'select_quotes_keywords', 'quotes_keywords' );
	}
