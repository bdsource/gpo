...
$document->addScript('/media/vendor/jquery/js/jquery.js');
...
if( $this->oMas->modified === '0000-00-00 00:00:00' || empty( $this->oMas->modified ) ) {
    ...
    $this->oMas->modified = strtotime($this->oMas->modified);
}
...
<form method="post" action="<?php echo JRoute::_('index.php?option=com_gpo');?>" id="adminForm" name="adminForm" style="padding-top: 0px;">
...
<div class="row">
    <div style="float:right;padding:0px;margin:0px auto;padding-right:50px;">
        <p>
            ...
            <input class="not-editable" style="width:40px;" type="text" id="mas_id" name="mas[id]" value="<?php echo $this->oMas->live_id; ?>" disabled="true" />
        </p>
    </div>
...
?>
 ...
 <div class="row">
    <div class="cell left_col">
        ...
        <textarea id="mas_narrative" name="mas[narrative]" style="width:632px;height: 220px" rows="<?php echo $narrativeRows;?>"><?php echo $this->oMas->narrative;?></textarea>
    </div>     
</div>
...
<div class="row">
    <div style="float:left;padding:0px;margin:0px auto;padding-right:50px;">
            <label style="display:block;" for="mas_modified" title="Date of publication. Book, 2008: 1/1/08. Aug/Sep 2008 Issue: 1/8/08. Early copy: date received, plus 'June issue,' 'Summer issue,' etc. in Notes">Date Modified*</label>
...
/*
//Allow the left click of the mouse to trigger a new item
$('select_mas_source').observe( 'change', function(event){
	if( this.selectedIndex != '0' )
	{
		select_split( 'select_mas_source', 'mas_source' );
	}
});
//this is required to reset the list options
$('mas_source').observe('focus',function(event){
	$('select_mas_source').selectedIndex = '0';
});

$('select_mas_source').observe( 'keypress', function(event){
	if( event.keyCode == Event.KEY_RETURN )
	{
		select_split( 'select_mas_source', 'mas_source' );
	}
...
$('select_mas_country').observe( 'change', function(event){
    if( this.selectedIndex != '0' )
    {
        location_select_add( this.readAttribute('id') );	
    }
});
$('select_mas_country').observe( 'keypress', function(event){
    if( event.keyCode == Event.KEY_RETURN )
    {
		location_select_add( this.readAttribute('id') );			
    }
...
$('select_mas_region').observe( 'change', function(event){
	if( this.selectedIndex != '0' )
	{
		location_select_add( this.readAttribute('id') );	
	}
});
$('select_mas_region').observe( 'keypress', function(event){
	if( event.keyCode == Event.KEY_RETURN )
	{
		location_select_add( this.readAttribute('id') );
	}
...
$('select_mas_keywords').observe( 'change', function(event){
	if( this.selectedIndex != '0' )
	{
		select_split( 'select_mas_keywords', 'mas_keywords' );
	}
});

//this is required to reset the list options

$('mas_keywords').observe('focus',function(event){
	$('select_mas_keywords').selectedIndex = '0';
});
$('select_mas_keywords').observe( 'keypress', function(event){
	if( event.keyCode == Event.KEY_RETURN )
	{
		select_split( 'select_mas_keywords', 'mas_keywords' );
	}
...
{
	Event.stop(event);
	str = $('mas_byline').getValue();
	str = str.toLowerCase();
	str = (str+'').replace(/^(.)|\s(.)/g, function ( $1 ) { return $1.toUpperCase( );} );
	$('mas_byline').value=str;
});

$('subtitle_ucwords').observe('click',function(event)
{
	Event.stop(event);
	str = $('mas_sub_title').getValue();
	str = str.toLowerCase();	
	str += '';
    var f = str.charAt(0).toUpperCase();
    nstr = f + str.substr(1);
	$('mas_sub_title').value=nstr;
});

$('gpnheader_ucwords').observe('click',function(event)
{
	Event.stop(event);
	str = $('mas_gpnheader').getValue();
	str = str.toLowerCase();
	str = (str+'').replace(/^(.)|\s(.)/g, function ( $1 ) { return $1.toUpperCase( );} );
	$('mas_gpnheader').value=str;
});

//$('content_remove_html').observe('click',function(event)
//{
//	Event.stop(event);
//	clean = $('mas_content' ).getValue().stripTags().stripScripts();
//	$('mas_content').update( clean );
//});
*/
...
<script type="text/javascript">
      ...
      jQuery('#select_mas_location').on('change',function(event)
       {
        var URL = ' JRoute::_("index.php?option=com_gpo&controller=mas&task=getStates", false); ?>';
...
<script type="text/javascript">
     jQuery( function() {
    jQuery( "#mas_date_of_shooting" ).datepicker({ 
        dateFormat: 'dd MM yy',
        changeMonth: true, 
        changeYear: true, 
        yearRange: "-90:+00"
