...
<span title="Mentions in Ncites">Mentions in NCites</span>
</h3>
<div class="jpane-slider content">
<?php 
$qCitesLink = 'index.php?option=com_gpo&controller=citations&type=mas&task=frt&action=add'.
...
?>
  ...
  <?php echo $this->ncites_items; ?> mentions of '<?php echo $from; ?>' were also found in the 
  '<?php echo $this->options['column_name'];?>' field of the Ncites table. 
  </span>
  To open the NCites search results in a new window, 
  <a href="<?php echo $qCitesLink?>" onClick="javascript: updateQciteStatus();"
     title='Click here to search in Ncites table: Opens in New Window' target="_blank">
    click here.
  </a>
</div>
</div>
...
<form action="index.php?option=com_gpo&controller=mas" method="post" name="adminForm" class="mas_Result" id="adminForm">
<div class="responsive">
<table class="yellow adminlist tablesorter table-striped table-hover table-bordered" id="adminlist" width="100%">
		...
		<tr class="<?php echo "row$k"; ?>">
			<td width="40" valign="top" align="center"> 
			    <a style="color:green" target="_blank" 
			       href="index.php?option=com_gpo&controller=mas&task=lookup&state=published&id=<?php echo $item->id;?>" 
			...
			<td width="50" valign="top" align="center"> 
			    <?php echo implode(', ', $this->masModel->getLocations( $item->country_id ) );?>
			</td>
...
function giveFnRWarning( ) {
	...
	var statusVar = 1;
	var qCitesWarning = "These strings have also been found in NCites. Before altering them in Mas, ";
	qCitesWarning	 += "open the NCites search results in a new window to ensure you make the same changes there.";
	var boxedCheckedVal = document.adminForm.boxchecked.value;
	
<?php 
    if( $this->ncites_items > 0 ): 
?>
      if ( qCiteClickStatus == 0 ){
        //show warning for qcites mentions
        //alert(qCitesWarning);
        new Messi('<p>'+qCitesWarning+'</p>', {title: 'Match Found in Ncites', modal: true});
        statusVar = 0;
      }
<?php
    endif;
