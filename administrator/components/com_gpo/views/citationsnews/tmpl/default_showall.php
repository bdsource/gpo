...
<?php
//	$link_view = $front_end . JRoute::_( 'index.php?option=com_gpo&task=article&id='. $row['id'] );
	$link_edit = JRoute::_( 'index.php?option=com_gpo&controller=citations&task=edit&live_id='. $row['id'] );
	$access = ( $row['public'] == 1 ) ? 'public' : 'members';
	//$published = ( $row['post'] == 1 ) ? 'published' : 'not published';
	$published = '&nbsp;';
?>
		<tr>
