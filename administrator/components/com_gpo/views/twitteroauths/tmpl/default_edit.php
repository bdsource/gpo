...
document.observe( "topic:populate", function( event )
{
	...
	$( 't_window_title' ).value = word + " - Daily Bulletin from AVR Monitor News";
	$( 't_page_headline' ).value = word + " - News Feed";
	$( 't_spiderbait' ).value = "Updated daily, this news feed points to mass-media articles on a single aspect of AVR Monitor. For earlier articles, other topics or regions, use Search.";
	$( 't_meta_author' ).value = "Philip Alpers";
	$( 't_meta_keywords' ).value = word.toLowerCase() + ", gun news, gun control, AVR Monitor, small arms news, small arms policy, small arms proliferation, firearm news, firearm violence, firearm law, public health, Philip Alpers";
	$( 't_meta_description' ).value = word + " - Daily Bulletin from AVR Monitor News";
});
