...
class modgposponsorshelper
{
    ...
    function getSponsors() {
        ...
        //$uri_object =  Uri::getInstance();
       // $request_uri = modgposponsorshelper::_filterUri($uri_object->toString(array('path')));
       $uri = Uri::getInstance();
       $request_uri = modgposponsorshelper::_filterUri($uri);
        ...
        $sponsors = modgposponsorshelper::_getSponsors($uri, $langCode);
        ...
        $ancestorSponsors = modgposponsorshelper::_lookForAncestorRegions($request_uri, $langCode);
    ...
    function _search( $uri )
    {
			...
			$url_search = "firearms/search";
			 $uri = Uri::getInstance();
			if( strpos($uri, $url_search ) === false )
			{
				return $uri;
			}
			$uri = "search";
    ...
    */
    ...
    function _filterUri( $request_uri ) {
			...
			$request_uri = modgposponsorshelper::_search( $request_uri );
    ...
    function _lookForAncestorRegions( $request_uri, $langCode=false ) {
    	...
    	$catAlias = modgposponsorshelper::_getRegionAliasFromURI( $request_uri );
    	$locationInfo = modgposponsorshelper::_getLocationInfoByAlias( $catAlias );
    
    	$ancestors = modgposponsorshelper::_getRegionAncestors( $locationInfo->id );
    	
    	foreach ( $ancestors as $arow) {
    	   if( empty($arow->alias) ) { continue; }
    	   $uri = modgposponsorshelper::_makeUri( $arow->alias, $langCode );
